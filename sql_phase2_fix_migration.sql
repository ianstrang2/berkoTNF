-- Phase 2 FIX: Corrected Data Migration - Calculate Own Goals for Historical Matches
...
WITH match_calculations AS (
  ...
  -- FIXED: Remove the WHERE clause that was preventing updates
...
-- Show summary of corrected migration results
SELECT 
...
SELECT 
  'Team A own goals total' as metric,
  SUM(team_a_own_goals)::text::int as count
FROM matches

UNION ALL

SELECT 
  'Team B own goals total' as metric,
  SUM(team_b_own_goals)::text::int as count
FROM matches; 