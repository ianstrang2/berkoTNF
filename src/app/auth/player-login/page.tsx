/**
 * Player Phone Authentication Page
 * 
 * /auth/player-login
 * Players sign up/login using phone number and SMS OTP
 */
  ...
  const handleVerifyOTP = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.session) {
        ...
        if (profileData.linkedPlayerId) {
          // Has profile, go to dashboard
          router.push('/');
        } else {
          // No profile, go to claiming flow
          router.push('/auth/claim-profile');
        }
      }
    } catch (error: any) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-700 to-pink-500 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-xl p-8 w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-gray-900">Player Login</h1>
