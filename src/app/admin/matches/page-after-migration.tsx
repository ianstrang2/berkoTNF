...
interface HistoricalMatch {
  match_id: number;
  upcoming_match_id: number; // No longer nullable after migration
  match_date: string;
  team_a_score: number;
  team_b_score: number;
  state: string;
...
const MatchListPageContent = () => {
  ...
  const renderHistoryList = () => (
    <div className="space-y-4 max-w-3xl">
      {history.map(match => {
        // After migration, ALL matches have upcoming_match_id, so all are clickable
        return (
          <Link key={match.match_id} href={`/admin/matches/${match.upcoming_match_id}`} className="block bg-white hover:shadow-lg transition-shadow duration-300 p-4 rounded-xl shadow-soft-xl border">
            <div className="flex justify-between items-center">
              <div>
                <p className="font-semibold text-slate-700">{format(new Date(match.match_date), 'EEEE, MMMM d, yyyy')}</p>
                <p className="text-sm text-slate-500 font-bold">{match.team_a_score} - {match.team_b_score}</p>
              </div>
              <span className="text-xs font-bold uppercase py-1 px-3 rounded-full bg-green-100 text-green-800">{formatStateDisplay(match.state || 'COMPLETED')}</span>
            </div>
          </Link>
        );
      })}
