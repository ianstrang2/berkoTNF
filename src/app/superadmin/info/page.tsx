...
export default function SuperadminInfoPage() {
  const { profile } = useAuthContext();
  const hasTenantContext = profile?.tenantId !== null && profile?.tenantId !== undefined;
  ...
  // Note: This page is now superadmin-only (moved from /admin/info)
  // It provides multi-tenant diagnostics and system monitoring
  ...
  useEffect(() => {
    // Always fetch platform-level data
    fetchCacheMetadata();
    fetchJobStatus();
    
    // Only fetch tenant-specific data if we have tenant context
    if (hasTenantContext) {
      fetchInfoData();
      fetchPlayers();
          ...
          {!hasTenantContext ? (
            <div className="mb-4 p-4 bg-yellow-50 border border-yellow-300 rounded-lg">
              <div className="flex items-start">
                ...
                <div>
                  <h3 className="text-sm font-semibold text-yellow-900 mb-1">No Tenant Selected</h3>
                  <p className="text-sm text-yellow-800">
                    You're in platform view. Some features (player profiles, absentees, EWMA ratings) require tenant context.
                  </p>
                  <p className="text-sm text-yellow-800 mt-2">
                    <strong>To view tenant-specific data:</strong> Click the profile dropdown → "View as Admin" → Select a tenant
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="mb-4 p-3 bg-green-50 border border-green-200 rounded-lg">
              <p className="text-sm text-green-800">
                ✅ <strong>Tenant context active:</strong> All features available for the selected tenant.
              </p>
            </div>
          )}
                  ...
                  {!hasTenantContext ? (
                    <div className="flex items-center justify-center h-full">
                      <div className="text-center text-slate-500">
                        ...
                        <p className="text-sm">Tenant context required</p>
                        <p className="text-xs mt-1">Switch to a tenant view to see player profiles</p>
                      </div>
                    </div>
                  ) : isLoadingProfiles ? (
                ...
                <div className="p-4">
                  {!hasTenantContext ? (
                    <div className="text-center text-slate-500 py-8">
                      <p className="text-sm">Tenant context required</p>
                      <p className="text-xs mt-1">Switch to a tenant view to see absentees</p>
                    </div>
                  ) : isLoadingInfoData ? (
              ...
              <div className="break-words bg-white border-0 shadow-soft-xl rounded-2xl bg-clip-border">
                ...
                <div className="p-4">
                  {!hasTenantContext ? (
                    <div className="text-center text-slate-500 py-8">
                      <p className="text-sm">Tenant context required</p>
                      <p className="text-xs mt-1">Switch to a tenant view to see ringers</p>
                    </div>
                  ) : isLoadingInfoData ? (
                ...
                <div className="p-4 border-b border-gray-200">
                  {!hasTenantContext ? (
                    <div className="text-center text-slate-500 py-4">
                      <p className="text-sm">Tenant context required to view EWMA ratings</p>
                    </div>
                  ) : (
                    <>
                      {playersError && (
