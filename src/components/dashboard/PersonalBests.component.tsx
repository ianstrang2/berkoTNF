...
const RecordsAndAchievements: React.FC = () => {
  ...
  const { data: pbData, isLoading: pbLoading, error: pbError } = usePersonalBests();
  const { data: milestonesData, isLoading: milestonesLoading } = useMatchReport();
  const { data: allPlayers = [], isLoading: playersLoading } = usePlayers();

  // Combined loading/error states
  const loading = pbLoading || milestonesLoading || playersLoading;
  const error = pbError;
  ...
  // Don't use useCallback for these - just inline functions to avoid dependency issues
  ...
  const processTimelineItems = (personalBestsData: PersonalBestsData | null, matchData: typeof milestonesData) => {
    ...
    if (matchData) {
      // Game milestones
      if (matchData.gamesMilestones && matchData.gamesMilestones.length > 0) {
        matchData.gamesMilestones.forEach((milestone: Milestone) => {
      ...
      // Goal milestones
      if (matchData.goalsMilestones && matchData.goalsMilestones.length > 0) {
        matchData.goalsMilestones.forEach((milestone: Milestone) => {
      ...
      if (matchData.featBreakingData && Array.isArray(matchData.featBreakingData)) {
        matchData.featBreakingData.forEach((feat: FeatBreakingItem) => {
  ...
  useEffect(() => {
    // Wait for all data to be defined (pbData can be null, that's valid - means no PBs broken)
    // But we need players loaded for player IDs
    const dataReady = pbData !== undefined && milestonesData !== undefined;
    const playersReady = !playersLoading;
    
    if (dataReady && playersReady) {
  ...
  // Only show empty state if there are no timeline items
  // (pbData can be null but we might still have milestones/feats from matchReport)
            ...
            {timelineItems.map((item, index) => (
              ...
              >
                <span className={`w-8 h-8 rounded-full text-base z-1 absolute left-4 inline-flex -translate-x-1/2 items-center justify-center bg-white dark:bg-gray-700 text-center font-semibold`}>
                  ...
                  {(item.icon === 'win_streak' || item.icon === 'scoring_streak') && (
                  ...
                  {item.icon === 'undefeated_streak' && (
                    <svg className={`w-5 h-5 ${item.color === 'green' ? 'text-green-500' : 'text-green-500'} dark:text-green-400`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
                      <path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75L11.25 15 15 9.75m-3-7.036A11.959 11.959 0 013.598 6 11.99 11.99 0 003 9.749c0 5.592 3.824 10.29 9 11.623 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.571-.598-3.751h-.152c-3.196 0-6.1-1.248-8.25-3.285z" />
                    </svg>
                  )}
                  {(item.icon === 'loss_streak' || item.icon === 'winless_streak') && (
