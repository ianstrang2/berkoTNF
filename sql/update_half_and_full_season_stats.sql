...
BEGIN
    ...
    WHERE p.is_ringer = false AND p.is_retired = false AND m.match_date BETWEEN half_season_start_date AND half_season_end_date
    ...
    LOOP
        ...
        JOIN matches m ON pm.match_id = m.match_id
        JOIN players p ON pm.player_id = p.player_id
        WHERE p.is_ringer = false AND p.is_retired = false AND EXTRACT(YEAR FROM m.match_date) = historical_year
        GROUP BY pm.player_id;
    END LOOP;
