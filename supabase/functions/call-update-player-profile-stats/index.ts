...
const FUNCTION_NAME = "call-update-player-profile-stats";
const TARGET_RPC = "update_aggregated_player_profile_stats";
...
serve(async (req: Request) => {
  ...
  try {
    ...
    // Initialize client directly with service role key
    supabase = createClient(supabaseUrl, serviceKey, {
      auth: {
        persistSession: false, // Recommended for server-side functions
        autoRefreshToken: false, // Recommended for server-side functions
        detectSessionInUrl: false // Recommended for server-side functions
      }
    });
  } catch (initError) {
    const errorMessage = initError instanceof Error ? initError.message : "Unknown client initialization error";
    console.error(`[${FUNCTION_NAME}] Supabase client initialization error:`, initError);
    return new Response(
      JSON.stringify({ success: false, error: `Client initialization error: ${errorMessage}` }),
